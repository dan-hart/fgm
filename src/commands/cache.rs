...
async fn warmup(file_key_or_url: &str, include_images: bool) -> Result<()> {
    ...
    println!(
        "{}",
        format!("Warming cache for file: {}", file_key).bold()
    );
    ...
    if include_images && !frame_ids.is_empty() {
        ...
        for chunk in frame_ids.chunks(50) {
            ...
            let _images = client.export_images(file_key, &chunk_vec, "png", 2).await;
    ...
    println!();
    println!("{}", "Cache warmed successfully:".green());
        ...
        if path.exists() {
            if let Ok(entries) = std::fs::read_dir(&path) {
                let total_size: u64 = entries
                    ...
                    .map(|m| m.len())
                    .sum();
                println!("  Disk usage: {} KB", total_size / 1024);
            }
        }
    }
...
async fn clear(all: bool, file_key: Option<&str>) -> Result<()> {
    ...
    if all {
        ...
        println!(
            "{}",
            format!("Cache cleared for file: {}", key).green()
        );
    } else {
        println!(
            "{}",
            "Specify --all to clear entire cache or --file <key> to clear specific file".yellow()
        );
        println!();
        println!("Examples:");
        println!("  fgm cache clear --all");
        println!("  fgm cache clear --file abc123");
    }
